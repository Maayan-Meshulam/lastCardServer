...
const loginUser = async (email, password) => {
  try {
    ...
    if(userFromDB.lockUntil && userFromDB.lockUntil > Date.now())
      return createError("Authentication", "Account is locked due mulitpale login attempts, try again later", 403)

    if (!comparePassword(password, userFromDB.password)) {
      const attempts = (userFromDB.loginAttempt || 0) + 1;  
      userFromDB.loginAttempt = attempts;   
